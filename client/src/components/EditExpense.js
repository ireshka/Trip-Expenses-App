...
class EditExpense extends Component {
  ...
  onEditSubmit = async (e) => {
    ...
    await axios.put(
      `/api/trips/${this.props.choosenTripId}/expenses/${this.props.choosenExpenseId}`,
      expense,
  ...
  getTripAndExpenseData = async () => {
    ...
    try {
      const sanitizedArrayCategories = res.data.categories.map((option) => ({
        value: option,
        label: option,
      }));
      let sanitizedExpense = res.data.expenses.filter(
        (arr) => arr._id === this.props.choosenExpenseId,
      );
