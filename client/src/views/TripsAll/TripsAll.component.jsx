...
class TripsAll extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`/api/users/${this.props.userId}/trips`, {
    ...
    if (!this.props.exchangeRates || this.props.exchangeRates.date !== todayDate) {
      getActualCurrencyRates(this.props.currencyList).then((res) => {
  ...
  render() {
    return (
      <ContentWrapper title="Your Trips">
        <LinkButtonBig to={`/trips/add`} color="green">
          Add a new trip
        </LinkButtonBig>
        <div>
